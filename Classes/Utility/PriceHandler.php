...
class PriceHandler
{
    ...
    public static function setPriceInCurrency(\TYPO3\CMS\Extbase\DomainObject\AbstractDomainObject $model, \S3b0\EcomConfigCodeGenerator\Domain\Model\Currency $currency = null, array $settings, $setNumericFields = ['noCurrencyPricing'], $setStringField = 'currencyPricing', $pricingField = 'pricing')
    {
        if ($currency instanceof \S3b0\EcomConfigCodeGenerator\Domain\Model\Currency) {
            ...
            $dec_point = $currency->isNumberSeparatorInUSFormat() ? '.' : ',';
            $thousands_sep = $currency->isNumberSeparatorInUSFormat() ? ',' : '.';
            if (!($model instanceof \S3b0\EcomConfigCodeGenerator\Domain\Model\Part && $model->getPartGroup()
                        ->isPricePercentage()) && $model->{$pricingField}->count()
            ) {
                /**
                ...
                if ($model->getPricingPercentage()) {
                    $numberValue = $model->getPartGroup()
                            ->getConfiguration()
                            ->getConfigurationPricingNumeric() * ($model->getPricingPercentage() / 100);
                } else {
                    $numberValue = 0.0;
                }
