...
//		}
	 ...
	 * @return void
	 */
	function itemsProcFuncEcomConfigCodeGeneratorDomainModelDependencyParts(array &$PA, $pObj = NULL)  {
		// Adding an item!
		...
		if ( sizeof($PA['items']) && $PA['row']['part_groups'] ) {
			...
			foreach ( $PA['items'] as $item ) {
				...
				$item[2] = 'clear.gif';
				$partGroupsCollection[0]['label'] = '-- not assigned --';
				if ( CoreUtility\MathUtility::canBeInterpretedAsInteger($data['partgroup']) ) {
					if ( !array_key_exists($data['partgroup'], $partGroupsCollection) ) {
						$configurationPackageLabels = BackendUtility\BackendUtility::getRecord('tx_ecomconfigcodegenerator_domain_model_partgroup', $data['partgroup'], 'title');
						$partGroupsCollection[$data['partgroup']]['label'] = $configurationPackageLabels['title'];
					}
